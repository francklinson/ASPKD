...
class Distributed:
    ...
    @classmethod
    def get_global_size(cls) -> int:
        """
        Returns:
            The number of processes in the process group
        """
        return dist.get_world_size() if Distributed.is_enabled() else 1
    ...
    def get_global_rank(cls) -> int:
        ...
        """
        return dist.get_rank() if Distributed.is_enabled() else 0
    ...
    @classmethod
    def _restrict_print_to_main_process(cls) -> None:
        """
        ...
        def print(*args, **kwargs):
            ...
            if Distributed.is_main_process() or force:
                builtin_print(*args, **kwargs)

        __builtin__.print = print

    @classmethod
    def _get_master_port(cls, seed: int = 0) -> int:
    ...
    @classmethod
    def _collect_env_vars(cls) -> Dict[str, str]:
        return {env_var: os.environ[env_var] for env_var in Distributed._TORCH_DISTRIBUTED_ENV_VARS if
                env_var in os.environ}

    @classmethod
    def _is_slurm_job_process(cls) -> bool:
        return "SLURM_JOB_ID" in os.environ

    @classmethod
    def _parse_slurm_node_list(cls, s: str) -> List[str]:
        nodes = []
    ...
    @classmethod
    def _check_env_variable(cls, key: str, new_value: str):
    ...
    @classmethod
    def enable(cls, *, set_cuda_current_device: bool = True, overwrite: bool = False, allow_nccl_timeout: bool = False):
        ...
        if allow_nccl_timeout:
            ...
            if not overwrite:
                Distributed._check_env_variable(key, value)
        ...
        Distributed._restrict_print_to_main_process()
...
class _TorchDistributedEnvironment:
    def __init__(self):
        ...
        if Distributed._is_slurm_job_process():
            return self._set_from_slurm_env()

        env_vars = Distributed._collect_env_vars()
        if not env_vars:
            # Environment is not set
            pass
        elif len(env_vars) == len(Distributed._TORCH_DISTRIBUTED_ENV_VARS):
    ...
    def _set_from_slurm_env(self):
        ...
        nodes = Distributed._parse_slurm_node_list(os.environ["SLURM_JOB_NODELIST"])
        assert len(nodes) == node_count

        self.master_addr = nodes[0]
        self.master_port = Distributed._get_master_port(seed=job_id)
    ...
    def _set_from_local(self):
        ...
        self.master_addr = "127.0.0.1"
        self.master_port = Distributed._get_available_port()
        self.rank = 0
        self.world_size = 1
        self.local_rank = 0
        self.local_world_size = 1
    ...
    def export(self, *, overwrite: bool) -> "_TorchDistributedEnvironment":
        ...
        if not overwrite:
            for k, v in env_vars.items():
                Distributed._check_env_variable(k, v)
