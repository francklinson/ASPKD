...
class GANLoss(nn.Module):
    def __init__(self, mode='hinge',
        ...
        self.lam = lam
        if self.mode not in ['bce', 'mse', 'hinge', 'wgan', 'logistic_saturating', 'logistic_nonsaturating',
                             'relativistic_gan']:
    ...
    def __call__(self, pred_fake=None, pred_real=None, isD=True):
        ...
        if self.mode == 'bce':
            ...
            if isD:
                loss_real = nn.BCEWithLogitsLoss()(pred_real - pred_fake.mean(0, keepdim=True),
                                                   torch.ones_like(pred_real))
                loss_fake = nn.BCEWithLogitsLoss()(pred_fake - pred_real.mean(0, keepdim=True),
...
class R1Loss(nn.Module):
    ...
    def forward(self, images, output):
        gradients = \
        autograd.grad(outputs=output, inputs=images, grad_outputs=torch.ones(output.size(), device=images.device),
