...
class InvADTrainer(BaseTrainer):
    ...
    def test(self):
        ...
        while batch_idx < test_length:
            ...
            # get anomaly maps
            anomaly_map, _ = self.evaluator.cal_anomaly_map(self.feats, self.feats_pred,
                                                            [self.imgs.shape[2], self.imgs.shape[3]],
                                                            uni_am=self.cfg.uni_am, use_cos=self.cfg.use_cos,
        ...
        if self.cfg.dist:
            ...
            if self.master:
                ...
                valid_results = False
                # 在分布式训练或并行计算场景中，等待并收集所有工作进程
                max_retries = 20
                retries = 0
                # 循环持续执行，直到所有结果文件被成功加载并验证有效。
                while not valid_results and retries < max_retries:
                    retries += 1
                    # 获取临时目录中所有以 .pth 为扩展名的文件。
                    results_files = glob.glob(f'{self.tmp_dir}/*.pth')
                    #  检查结果文件数量是否与分布式训练的进程数
                    if len(results_files) != self.cfg.world_size:
                        ...
                        while idx_result < self.cfg.world_size:
                            ...
                            try:
                                ...
                                time.sleep(1)
                        # 当所有文件都被成功加载后，设置 valid_results = True 退出循环。
                        valid_results = True
                if not valid_results:
                    raise RuntimeError(f'Failed to collect results from all processes after {max_retries} retries.')
        else:
            results = dict(imgs_masks=imgs_masks, anomaly_maps=anomaly_maps, cls_names=cls_names, anomalys=anomalys)
        if self.master:
