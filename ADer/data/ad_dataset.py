...
class RDPPAD(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        return {'img': img, 'img_noise': img_noise, 'img_mask': img_mask, 'cls_name': cls_name, 'anomaly': anomaly,
                'img_path': img_path}
...
class CifarAD(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        img_mask = Image.fromarray(
            np.zeros((img.size[0], img.size[1])) if anomaly == 0 else np.ones((img.size[0], img.size[1])), mode='L')
        img = self.transform(img) if self.transform is not None else img
        img_mask = self.target_transform(
            img_mask) if self.target_transform is not None and img_mask is not None else img_mask
        img_mask = [] if img_mask is None else img_mask
...
class TinyINAD(data.Dataset):
    def __init__(self, cfg, train=True, transform=None, target_transform=None):
        ...
        for i, pseudo_cls_name in enumerate(pseudo_cls_names):
            train_list.extend([[f'train/{pseudo_cls_name}/images/{file}', pseudo_cls_name] for file in
                               os.listdir(f'{self.root}/train/{pseudo_cls_name}/images')])
            test_list.extend([[f'val/{pseudo_cls_name}/images/{file}', pseudo_cls_name] for file in
                              os.listdir(f'{self.root}/val/{pseudo_cls_name}/images')])
        # assign data
...
class Draem(data.Dataset):
    ...
    def augment_image(self, image, anomaly_source_path):
        ...
        perlin_noise = self.rand_perlin_2d_np((self.resize_shape[0], self.resize_shape[1]),
