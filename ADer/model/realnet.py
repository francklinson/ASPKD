    ...
    def __init__(self,
        ...
        if self.backbone == 'resnet50' or self.backbone == 'wide_resnet50_2':
            ...
            raise NotImplementedError(
                "backbone must in [resnet18, resnet34,resnet50, wide_resnet50_2, efficientnet_b4]")
...
class AFS(nn.Module):
    def __init__(self,
        ...
        for block in self.structure:
            for layer in block['layers']:
                self.indexes["{}_{}".format(block['name'], layer['idx'])] = nn.Parameter(
    ...
    def forward(self, inputs, train=False):
        ...
        for block in self.structure:
            ...
            for layer in block['layers']:
                feat_c = torch.index_select(feats[layer['idx']]['feat'], 1,
                                            self.indexes["{}_{}".format(block['name'], layer['idx'])].data)
        ...
        if train:
            ...
            for block in self.structure:
                ...
                for layer in block['layers']:
                    feat_c = torch.index_select(gt_feats[layer['idx']]['feat'], 1,
                                                self.indexes["{}_{}".format(block['name'], layer['idx'])].data)
                    feat_c = getattr(self, "{}_{}_upsample".format(block['name'], layer['idx']))(feat_c)
...
class ReconstructionLayer(nn.Module):
    ...
    def forward(self, inputs, train=False):
        ...
        recon_feats = {block_name: getattr(self, '{}_recon'.format(block_name))(block_feats[block_name]) for block_name
                       in block_feats}
...
class RRS(torch.nn.Module):
    def __init__(self,
        ...
        for block in self.instrides:
            self.add_module("{}_upsample".format(block),
                            nn.UpsamplingBilinear2d(scale_factor=self.instrides[block] / align_stride))
...
class RealNet(nn.Module):
    def __init__(self, model_backbone, model_afs, model_recon, model_rrs, data_cfg):
        ...
        self.net_afs = AFS(inplanes=self.net_backbone.get_outplanes(), instrides=self.net_backbone.get_outstrides(),
                           structure=model_afs.structure, init_bsn=model_afs.init_bsn)
        self.net_recon = ReconstructionLayer(inplanes=self.net_afs.get_outplanes(),
                                             instrides=self.net_afs.get_outstrides(), **model_recon.kwargs)
        self.net_rrs = RRS(inplanes=self.net_recon.get_outplanes(), instrides=self.net_recon.get_outstrides(),
                           **model_rrs.kwargs)
