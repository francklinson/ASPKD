            ...
            for range_start in [0, 1]:
                ...
                for start_idx in range(range_start, num_level, 2):
                    if start_idx + 2 < num_level:
                        ...
                        modules.append(
                            FlowBlock(self.channel, direct='down', start_level=start_idx, ksize=ksize, vn_dims=vn_dims))
...
class PYRAMIDFLOW(torch.nn.Module):
    def __init__(self, model_backbone, batchsize):
        ...
        self.net_pyramidflow = PyramidFlow(resnet=self.model_backbone, channel=64, num_level=4, num_stack=4, ksize=7,
                                           vn_dims=(0, 1), batchsize=batchsize, savemem=False)
