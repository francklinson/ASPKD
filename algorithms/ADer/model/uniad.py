...
class UniAD(nn.Module):
    def __init__(self, model_backbone, model_decoder):
        ...
        self.net_merge = MFCN(inplanes=model_decoder['inplanes'], outplanes=model_decoder['outplanes'],
                              instrides=[2, 4, 8, 16], outstrides=[16])
        self.net_ad = UniAD_decoder(inplanes=model_decoder['outplanes'], instrides=model_decoder['instrides'],
                                    feature_size=model_decoder['feature_size'],
                                    feature_jitter=Namespace(**{'scale': 20.0, 'prob': 1.0}),
                                    neighbor_mask=Namespace(**{'neighbor_size': model_decoder['neighbor_size'],
                                                               'mask': [True, True, True]}),
...
if __name__ == '__main__':
    ...
    model_backbone.kwargs = dict(pretrained=False,
                                 checkpoint_path='model/pretrain/efficientnet_b4_ra2_320-7eb33cd5.pth', strict=False,
