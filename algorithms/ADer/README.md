...
> - <span style="color:red">**ADer**</span> is an open source visual **A**nomaly **D**etection toolbox based on PyTorch,
    which supports multiple popular AD datasets and approaches. </br>
> - We reproduce popular AD methods under the **M**ulti-class **U**nsupervised **A**nomaly **D**
    etection (<span style="color:red">**MUAD**</span>) by default. </br>
...
- 🔥 Plain ViT based <span style="color:red">**ViTAD**</span> is accepted by CVIU'25 🐲 [**Paper
  **](https://arxiv.org/abs/2312.07495) | [**Project**](https://zhangzjn.github.io/projects/ViTAD) | [**Code
  **](https://github.com/zhangzjn/ader/configs/vitad)
- 🔥 We have released several reproduced models, configuration files, and training logs in our <span style="color:red">*
  *benchmark**</span> paper 🐲 [**Paper**](https://arxiv.org/abs/2406.03262) | [**Results & CFGs
  **](configs/benchmark/README.md)
- 🔥 <span style="color:red">**COCO-AD**</span> and powerful <span style="color:red">**InvAD**</span> is released 🐲 [*
  *Paper**](https://arxiv.org/abs/2404.10760) | [**Project**](https://zhangzjn.github.io/projects/InvAD) | [**Code
  **](https://github.com/zhangzjn/ader/configs/invad)
- 🔥 <span style="color:red">**Real-IAD**</span> is released: a new large-scale challenging industrial AD dataset 🐲 [*
  *Paper**](https://arxiv.org/abs/2403.12580) | [**Project**](https://realiad4ad.github.io/Real-IAD) | [**Code
  **](https://github.com/TencentYoutuResearch/AnomalyDetection_Real-IAD)
...
- [x] Reproduced popular methods in [ADer Benchmark](https://arxiv.org/abs/2406.03262):
    ...
    - 🚀Reconstruction-based
        ...
        - [ ] [DiAD, AAAI'24](https://github.com/lewandofskee/DiAD): See tripartite implementation in
          this [website](https://github.com/lewandofskee/DiAD)
...
- [x] 🚀 (Opt.) DDP Training
- [x] By default, the weights used by different methods are automatically downloaded (`model.kwargs.pretrained=True`,
  `model.kwargs.checkpoint_path=''`). If you prefer to specify offline weights, you can download the model weights to
  `model/pretrain` and modify the settings to `model.kwargs.pretrained=False`,
...
- Prepare general experimental environment
  ```shell
  pip3 install timm==0.8.15dev0 mmselfsup pandas transformers openpyxl imgaug numba tensorboard fvcore accimage Ninja
  pip3 install mmdet==2.25.3
  pip3 install --upgrade protobuf==3.20.1 scikit-image faiss-gpu
  pip3 install adeval
  pip3 install torch==2.1.2 torchvision==0.16.2 torchaudio==2.1.2 --index-url https://download.pytorch.org/whl/cu118
  pip3 install numpy==1.26.4 tensorboardX fvcore==0.1.5.post20221221 numpy-hilbert-curve==1.0.1
  pip3 install fastprogress geomloss FrEIA mamba_ssm adeval  imgaug numba scikit-sklearn pyzorder
...
- [x] [Real-IAD](data/README.md/###Real-IAD): A new large-scale challenging industrial AD dataset, containing 30 classes
  with totally 151,050 images; 2,000 ∼ 5,000 resolution; 0.01% ~ 6.75% defect proportions; 1:1 ~ 1:10 defect ratio.
...
CUDA_VISIBLE_DEVICES=0 python run.py -c configs/vitad/vitad_mvtec.py -m train
CUDA_VISIBLE_DEVICES=0 python run.py -c configs/vitad/vitad_spk.py -m train

CUDA_VISIBLE_DEVICES=0 python run.py -c configs/mambaad/mambaad_mvtec.py -m train
CUDA_VISIBLE_DEVICES=0 python run.py -c configs/mambaad/mambaad_spk.py -m train

CUDA_VISIBLE_DEVICES=0 python run.py -c configs/invad/invad_mvtec.py -m train
CUDA_VISIBLE_DEVICES=0 python run.py -c configs/invad/invad_spk.py -m train

CUDA_VISIBLE_DEVICES=0 python run.py -c configs/uniad/uniad_mvtec.py -m train
CUDA_VISIBLE_DEVICES=0 python run.py -c configs/uniad/uniad_spk.py -m train

CUDA_VISIBLE_DEVICES=0 python run.py -c configs/benchmark/cflow/cflow_256_100e.py -m train
CUDA_VISIBLE_DEVICES=0 python run.py -c configs/benchmark/cfa/cfa_256_100e.py -m train
CUDA_VISIBLE_DEVICES=0 python run.py -c configs/benchmark/draem/draem_256_100e.py -m train
...
- Single-class Unsupervised AD (SUAD, not recommend currently)
    1. one `GPU-0` for training one `grid` class in `mvtec` dataset:
       `CUDA_VISIBLE_DEVICES=0 python3 run.py -c configs/vitad/single_cls/vitad_mvtec_bs16.py -m train data.cls_names=grid trainer.checkpoint=runs/vitad/single_class/vitad_mvtec_bs16/grid`
    2. one `GPU-0` for training all classes serially in `mvtec` dataset:

  python runs_single_class.py -d mvtec -c configs/vitad/vitad_mvtec.py -n 1 -m -1 -g 0
  python runs_single_class.py -d mvtec -c configs/mambaad/mambaad_mvtec.py -n 1 -m -1 -g 0

    3. `$GPU_NUM` GPUs for training all classes parallelly in `mvtec` dataset::
       `python3 runs_single_class.py -d mvtec -c configs/vitad/single_cls/vitad_mvtec_bs16.py -n $GPU_NUM -m 1`
...
CUDA_VISIBLE_DEVICES=0 python ADer/run.py -c ADer/configs/mambaad/mambaad_mvtec.py -m test vis=True vis_dir=vis
CUDA_VISIBLE_DEVICES=0 python run.py -c configs/mambaad/mambaad_spk.py -m test vis=True vis_dir=vis

CUDA_VISIBLE_DEVICES=0 python run.py -c configs/vitad/vitad_mvtec.py -m test vis=True vis_dir=vis
CUDA_VISIBLE_DEVICES=0 python run.py -c configs/vitad/vitad_spk.py -m test vis=True vis_dir=vis

CUDA_VISIBLE_DEVICES=0 python run.py -c configs/invad/invad_mvtec.py -m test vis=True vis_dir=vis
CUDA_VISIBLE_DEVICES=0 python run.py -c configs/invad/invad_spk.py -m test vis=True vis_dir=vis

CUDA_VISIBLE_DEVICES=0 python run.py -c configs/uniad/uniad_mvtec.py -m test vis=True vis_dir=vis
CUDA_VISIBLE_DEVICES=0 python run.py -c configs/uniad/uniad_spk.py -m test vis=True vis_dir=vis

CUDA_VISIBLE_DEVICES=0 python run.py -c configs/benchmark/draem/draem_256_100e.py -m test vis=True vis_dir=vis
CUDA_VISIBLE_DEVICES=0 python run.py -c configs/benchmark/cflow/cflow_256_100e.py -m test vis=True vis_dir=vis
