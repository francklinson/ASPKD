...
class SimpleNetTrainer(BaseTrainer):
    def __init__(self, cfg):
        ...
        self.optim.proj_opt = get_optim(cfg.optim.proj_opt.kwargs, self.net.net_simplenet.pre_projection,
                                        lr=cfg.optim.lr * .1)
        self.optim.dsc_opt = get_optim(cfg.optim.dsc_opt.kwargs, self.net.net_simplenet.discriminator,
                                       lr=cfg.optim.lr * .2)
    ...
    def backward_term(self, loss_term, optims, params):
        for i, (optim, param) in enumerate(zip(optims, params)):
            ...
            optim.zero_grad()
            if self.loss_scaler:
                self.loss_scaler(loss_term, optim, clip_grad=self.cfg.loss.clip_grad, parameters=param,
    ...
    def optimize_parameters(self):
        ...
        update_log_term(self.log_terms.get('sum'), reduce_tensor(loss_cos, self.world_size).clone().detach().item(), 1,
                        self.master)
    ...
    def test(self):
        ...
        while batch_idx < test_length:
            ...
            # self.net.predict()
            loss_cos = self.loss_terms['sum'](self.true_loss, self.fake_loss)
            update_log_term(self.log_terms.get('sum'), reduce_tensor(loss_cos, self.world_size).clone().detach().item(),
                            1, self.master)
            # get anomaly maps
