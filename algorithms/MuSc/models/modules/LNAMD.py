...
class PatchMaker:
    def __init__(self, patch_size, stride=None):
        ...
        实现原理
            初始化 (__init__):
                patch_size: 定义每个补丁的大小（例如 3x3）。
                stride: 定义补丁之间的步长，默认为 None（通常与 patch_size 相同）。
            补丁生成 (patchify):
        ...
        注意事项
            如果 stride 未设置，默认为 patch_size，可能导致补丁之间没有重叠。
            return_spatial_info 参数可以返回补丁的空间维度信息，便于后续处理。
        """
        self.patch_size = patch_size  # 设置patch大小
        self.stride = stride  # 设置stride大小

    def patchify(self, input_features, return_spatial_info=False):

        """
        将输入特征图转换为patch形式

        参数:
            input_features: 输入的特征图
        ...
        返回:
            ...
            否则只返回patch
        """
        padding = int((self.patch_size - 1) / 2)  # 计算填充大小
        # 创建Unfold层用于提取patch
        unfolder = torch.nn.Unfold(
            kernel_size=self.patch_size, stride=self.stride, padding=padding, dilation=1
        )
        unfolded_features = unfolder(input_features)  # 应用Unfold提取patch
        number_of_total_patches = []  # 用于存储每个维度上的patch数量
        # 计算每个维度上的patch数量
        for _s in input_features.shape[-2:]:
            n_patches = (
                                _s + 2 * padding - 1 * (self.patch_size - 1) - 1
                        ) / self.stride + 1
            number_of_total_patches.append(int(n_patches))
        # 重塑张量形状并调整维度顺序
        unfolded_features = unfolded_features.reshape(
            *input_features.shape[:2], self.patch_size, self.patch_size, -1
        )
...
class Preprocessing(torch.nn.Module):
    ...
    def forward(self, input_features):
        ...
        # 遍历每个预处理模块和对应的输入特征
        for module, feature in zip(self.preprocessing_modules, input_features):
...
class MeanMapper(torch.nn.Module):
    ...
    def forward(self, input_features):
        ...
        input_features = input_features.reshape(len(input_features), 1, -1)

        # 使用自适应平均池化将特征长度维度缩减到preprocessing_dim
        return F.adaptive_avg_pool1d(input_features, self.preprocessing_dim).squeeze(1)  # 移除维度为1的维度


class LNAMD(torch.nn.Module):
    def __init__(self, compute_device, feature_dim=1024, feature_layer=None, r=3, patchstride=1):
        """
        初始化LNAMD模块
        参数:
            compute_device: 计算设备(CPU/GPU)
    ...
    def _embed(self, input_feature_list):
        ...
        if input_feature_list is None:
            raise ValueError('input_feature_list cannot be None')
        batch_size = input_feature_list[0].shape[0]  # 获取批次大小

        features_layers = []
        for feature in input_feature_list:
            # reshape and layer normalization
            if feature.shape[1] < 2:
                raise ValueError('Feature must have at least 2 tokens')
        ...
        features_layers = features_layers.reshape(batch_size, -1, *features_layers.shape[-2:])  # (B, L, layer, C)
...
if __name__ == "__main__":
    ...
    device = 'cuda:0'
    LNAMD_r = LNAMD(compute_device=device, r=3, feature_dim=1024, feature_layer=[1, 2, 3, 4])
    B = 32
