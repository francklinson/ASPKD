...
from hiad.utils import metrics, metrics_gpu
...
from hiad.detectors import HRDesTSeg
...
if __name__ == '__main__':
    ...
    parser.add_argument("--image_size", default=2048, type = int, help='Detection resolution: images will be resized to this size for detection')
    parser.add_argument("--patch_size", default=512, type = int, help='Patch size: size of the image patches after cropping')
    parser.add_argument("--stride", default=-1, type = int,  help='Stride size: set to -1 to use the same value as the patch size')
    ...
    parser.add_argument("--thumbnail_size", default = 512, type=int, help='low-resolution detection resolution')
    parser.add_argument("--thumbnail_config", default = 'configs/destseg.yaml', type=str, help='Low-res(thumbnail) detection model configuration path')
    parser.add_argument("--detector_config_path", default = 'configs/destseg.yaml', type = str, help='High-res detection model configuration path')
    ...
    parser.add_argument("--early_stop_epochs", default = 8, type = int,
    ...
    parser.add_argument("--checkpoint_root", default ='results/destseg_checkpoints', type = str, help='Directory for saving model checkpoints')
    parser.add_argument("--log_root", default = 'results/destseg_logs', type = str, help='Directory for saving logs')
    parser.add_argument("--vis_root", default = 'results/destseg_vis', type = str, help='Directory for saving visualization results')
    ...
    train_samples = [HRSample(image=os.path.join(args.data_root, sample['filename']),
                              ...
                              foreground=os.path.join(args.data_root, sample['foreground']) if 'foreground' in sample else None,
    ...
    if not args.evaluation_only:
        ...
        tasks = SpatialClusteringTaskGenerator( detector_class, config, args.batch_size,
                                                cluster_number= detector_number,
                                                fusion_weights=args.fusion_weights,
                                                feature_resolution = 1, device = args.gpus ).create_tasks(train_samples, image_indexes)

        # tasks = RetrieveTaskGenerator(detector_class, config, args.batch_size,
        #                                        cluster_number = args.detector_number,
        #                                        fusion_weights=args.fusion_weights,
        #                                        feature_resolution = 1, device = args.gpus).create_tasks(train_samples, image_indexes)
    ...
    if not args.evaluation_only:
        ...
        val_config = {
            'anomaly_syn_fn': RandomeBoxSynthesizer(p=0.5, diff_threshold = [20, 50]),
            'val_sample_ratio': 0.2,
            'max_patch_number': 200,
