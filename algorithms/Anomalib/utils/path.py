...
def get_pretrained_weights_dir() -> Path:
    """Return the pre-trained model weights directory.

    First checks for the project-local ``models/pre_trained`` directory
    (relative to the project root, detected by walking up from this file).
    Falls back to the platform-appropriate cache directory
    (``~/.cache/anomalib/pre_trained``) if the project directory is not found.

    Returns:
        Path: Path to the pre-trained weights directory.
    ...
    Example:
        ...
        'pre_trained'
    """
    # Try project-local models/pre_trained first
    current_file = Path(os.path.abspath(__file__))
    # Walk up to find project root (where models/ lives)
    for parent in [current_file] + list(current_file.parents):
        candidate = parent / "models" / "pre_trained"
        if candidate.is_dir():
