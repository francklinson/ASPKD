...
class AnomalibModule(ExportMixin, pl.LightningModule, ABC):
    ...
    def on_train_start(self) -> None:
        """Ensure the model is in training mode when training begins.

        Lightning 2.x does not automatically call ``model.train()`` before the
        training step. When the model is set to eval mode (e.g. by ``load_model``)
        before training, the inner model stays in eval mode, causing models like
        PatchCore/PaDiM that rely on ``self.training`` in ``forward()`` to fail
        with ``Memory bank is empty`` errors.
        """
        self.train()
