...
if __name__ == "__main__":
    ...
    for idx, target_dir in enumerate(dirs):
        ...
        # load data 
        gen_train = DataGenerator(list_files_npy_train, batch_size=param["fit"]["batch_size"],
                                  dim=(shape0_feat, shape1_feat), step=param["step"])
        gen_val = DataGenerator(list_files_npy_val, batch_size=param["fit"]["batch_size"],
                                dim=(shape0_feat, shape1_feat), shuffle=False, step=param["step"])

        # train model
        print("============== MODEL TRAINING ==============")

        # checkpoint
        model_checkpoint = ModelCheckpoint(best_model_filepath + "{epoch:02d}.hdf5", monitor='val_loss', verbose=1,
                                           save_best_only=True)
