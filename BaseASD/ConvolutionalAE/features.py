"""
Script for extract and save features
"""
import os
import glob
import sys
...
def list_to_vector_array(file_list, feat_path,  scaler,
                         msg="calc...",
                         n_mels=64,
                         frames=5,
                         n_fft=1024,
    ...
    for idx in tqdm(range(len(file_list)), desc=msg):
        vector_array = com.file_to_vector_array(file_list[idx], scaler,
                                                n_mels=n_mels,
                                                frames=frames,
                                                n_fft=n_fft,
    ...
    #uncomment to compute scaler
    #save scaler
    scaler_file_path = "scalers/{machine_type}".format(machine_type=machine_type)
    # make scaler directory
    os.makedirs(scaler_file_path, exist_ok=True)
    scaler_file_path = os.path.abspath(scaler_file_path)
    # 如果不存在，则保存
    if not os.path.exists(scaler_file_path+"/scaler_{machine_type}.bin".format(machine_type=machine_type)):
...
if __name__ == "__main__":
    ...
    for idx, target_dir in enumerate(dirs):
        ...
        list_to_vector_array(train_filenames, features_dir_train, scaler=None,
        ...
        list_to_vector_array(val_filenames, features_dir_val,  scaler=None,
