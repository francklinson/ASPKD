...
def plot_ghost_ground_truth():
    ...
    # 保存为png
    plt.imsave('data/all_black.png', ghost_data)
    # 等待1s
    time.sleep(1)
    # 转换为灰度图
    convert_to_gray()
    # 检查是否生成幽灵数据,有没有ghost.png
    if not os.path.exists('data/ghost.png'):
        raise FileNotFoundError("data/ghost.png not generated successfully!!! check the code!!!")
...
def convert_to_gray():
    ...
    # 读取彩色图像并转换为灰度
    img = Image.open('data/all_black.png').convert('L')  # 'L'模式表示8位灰度
    # 修改为600*600大小
    img = img.resize((600, 600))
    # 保存为灰度图像
    img.save('data/ghost.png')
...
class Preprocessor:
    ...
    def process_audio_parallel(self, file_list, save_dir):
        ...
        # 导入并行接口
        try:
            from backend.core.shazam import ParallelAudioFingerprinter, ParallelResult
        except ImportError:
